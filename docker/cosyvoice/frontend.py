...
class CosyVoiceFrontEnd:
            ...
            for spk_id, info in REGISTER_SPK_INFO_DICT.items():
                try:
                    ...
                    if not os.path.exists(prompt_audio_path):
                        logging.error(
                            f"doesn't exist prompt_audio_path: {prompt_audio_path}"
                        )
                        continue
                    ...
                    self.generate_spk_info(
                        spk_id,
                        prompt_text,
                        prompt_audio,
                        self.feat_extractor.keywords["sampling_rate"],
                    )  # noqa
    ...
    def _extract_text_token(self, text):
        if isinstance(text, Generator):
            ...
            logging.info(
                "get tts_text async generator, will return _async_extract_text_token_generator!"
            )
            # NOTE add a dummy text_token_len for compatibility
            return self._async_extract_text_token_generator(text), torch.tensor(
                [0], dtype=torch.int32
            )
        else:
    ...
    def add_spk_info(self, spk_id: str, spk_info: Union[dict, SpeakerInfo]):
        ...
        torch.save(
            self.spk2info[spk_id], os.path.join(self.spk2info_path, spk_id + ".pt")
        )
    ...
    def load_spk_info(self, spk_id: str):
        if spk_id not in self.spk2info:
            ...
            spk_info = torch.load(
                spk_info_path, map_location=self.device, weights_only=False
            )
            self.spk2info[spk_id] = spk_info
